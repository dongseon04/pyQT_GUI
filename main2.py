...
class ParkingApp(QDialog):
    def __init__(self):
        ...
        # YOLOv5 모델 로드
        self.model = torch.hub.load('ultralytics/yolov5', 'custom', path='best.pt')  # 'best.pt'는 포트홀 감지용 가중치 파일
        ...
        def update_frame():
            if self.timer_active:
                ...
                if ret:
                    ...
                    annotated_frame = np.squeeze(results.render())  # 감지된 객체를 시각화한 프레임
                    ...
                    for detection in detections:
                        ...
                        print(f"감지된 객체: {label}, 위치=({x1:.0f}, {y1:.0f}, {x2:.0f}, {y2:.0f})")
        ...
        if name and phone and os.path.exists(image_file):  # 이름, 전화번호, 사진이 있는지 확인
            ...
            try:
                with open("parking_data.csv", "a", newline="", encoding="utf-8-sig") as file:
                    ...
                    if not file_exists:
                        writer.writerow(["이름", "전화번호", "메모", "사진", "감지된 객체"])
                    ...
                    writer.writerow([name, phone, memo, image_file, ", ".join(detected_objects)])
                print(f"저장된 데이터: 이름={name}, 전화번호={phone}, 메모={memo}, 사진={image_file}, 감지된 객체={', '.join(detected_objects)}")
